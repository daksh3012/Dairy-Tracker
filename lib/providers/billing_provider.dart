...
class BillingProvider with ChangeNotifier {
  ...
  Future<void> loadBills() async {
    ...
    try {
      ...
      final deliveries = MockDataService.generateMockDeliveries(customers, products, 500);
  ...
  Future<bool> generateBill(String customerId, DateTime billDate, DateTime dueDate) async {
    _isLoading = true;
    _errorMessage = null;
    notifyListeners();
  ...
  void _applyFilters() {
    _filteredBills = _bills.where((bill) {
      ...
      // Date range filter
      if (_selectedDateFrom != null && bill.billDate.isBefore(_selectedDateFrom!)) {
        return false;
      }
      ...
      // Customer filter
      if (_selectedCustomerId.isNotEmpty && bill.customerId != _selectedCustomerId) {
        return false;
      }
  ...
  Map<String, dynamic> get billingStats {
    ...
    final pendingAmount = _bills.fold(0.0, (sum, bill) => sum + bill.balanceAmount);
